...
sub meta_pack {
   ...
   my $attr = $self->{_meta_cache} || {};

   $attr->{mtime} = $mtime if defined $mtime;
...
sub _get_store_from_extension {
   ...
   my $stores = $self->_stores;

   return $stores->{$extn} if exists $stores->{$extn};

   my $list  = map_extension2class($extn);
   my $class = $list->[0] if $list;

   throw 'Extension [_1] has no class', [$extn] unless $class;
