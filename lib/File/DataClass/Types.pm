...
subtype Cache, as Object,
   where   { $_->isa('File::DataClass::Cache') || $_->isa('Class::Null') },
...
subtype Lock, as Object,
   where   { ($_->can('set') && $_->can('reset')) || $_->isa('Class::Null') },
...
sub _exception_message_for_object_reference {
   return inflate_message('String [_1] is not an object reference', $_[0]);
}

sub _exception_message_for_cache {
...
sub _exception_message_for_lock {
   return inflate_message(
...
sub _exception_message_for_path {
   return inflate_message(
...
sub _exception_message_for_result {
   return inflate_message(
